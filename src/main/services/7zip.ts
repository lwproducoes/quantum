...
export class SevenZip {
  private static readonly binaryPath = app.isPackaged
    ...
    : path.join(__dirname, '..', '..', 'binaries', binaryName[process.platform])
  ...
  public static extractFile(
    ...
    return new Promise((resolve, reject) => {
      const tryPassword = (index = -1) => {
        ...
        logger.info(`Trying password "${password || '(empty)'}" on ${filePath}`)
        ...
        stream.on('end', () => {
          logger.info(`Successfully extracted ${filePath} (${extractedFiles.length} files)`)
          resolve({
            success: true,
            extractedFiles
          })
        ...
        stream.on('error', (err) => {
          ...
          if (index < passwords.length - 1) {
            ...
            logger.error(`Failed to extract file: ${filePath} after trying all passwords`)
  ...
  public static listFiles(filePath: string, password?: string): Promise<string[]> {
    return new Promise((resolve, reject) => {
      const files: string[] = []
